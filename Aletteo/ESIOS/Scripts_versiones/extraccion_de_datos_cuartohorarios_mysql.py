...
for indicator_id in indicator_ids:
    ...
    url = f"{BASE_URL}/{indicator_id}?start_date={start_date}&end_date={end_date}&time_trunc=fifteen_minutes"
    try:
...
cursor.execute("""
CREATE TABLE IF NOT EXISTS Extracciones_cuartohorarias (
    Fecha DATETIME PRIMARY KEY
);
""")

# 📌 **Verificar columnas de la tabla que queremos en la base de datos**
cursor.execute("SHOW COLUMNS FROM Extracciones_cuartohorarias;")
existing_columns = {col[0] for col in cursor.fetchall()}

# 📌 **Añadir nuevas columnas si no existen**
for column in df.columns[1:]:
    if column not in existing_columns:
        cursor.execute(f"ALTER TABLE Extracciones_cuartohorarias ADD COLUMN `{column}` FLOAT NULL;")
...
for _, row in df.iterrows():
    ...
    insert_sql = f"""
    INSERT INTO Extracciones_cuartohorarias (Fecha, {columns})
...
cursor.close()
mysql_conn.close()
print("\n✅ Datos guardados en la base de datos MySQL en la tabla Extracciones_cuartohorarias.")
