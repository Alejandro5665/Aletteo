...
# 🔹 Obtener las columnas de la tabla de origen (`Extracciones_horarias_tratadas`)
query_columnas_origen = "SHOW COLUMNS FROM Extracciones_horarias_tratadas;"
...
print("📌 Columnas en la tabla 'Extracciones_horarias_tratadas' (sin 'Fecha'):", columnas_origen)

# 🔹 Obtener las columnas de la tabla de destino (`calculo_hueco_termico`)
...
print("📌 Columnas finales en 'calculo_hueco_termico':", columnas_destino)

# 🔹 Cargar datos de la tabla de origen con filtro de fechas
query_datos = f"""
    SELECT * FROM Extracciones_horarias_tratadas
...
with engine.begin() as connection:
    connection.execute(text("DELETE FROM calculo_hueco_termico;"))
    print("🗑️ Se han eliminado todos los datos previos en 'calculo_hueco_termico'.")

# 🔹 Insertar datos en MySQL correctamente
df_tratado.to_sql(
    "calculo_hueco_termico",
    con=engine,
